...
def main():
    ...
    print_question(die_1, die_2, questiontype)
    response = input_answer()
    if response == -1:
        print_error_message()
    else:
        if is_correct_answer(die_1, die_2, questiontype, response) is True:
            print_congratulations(questiontype)
        else:
            print_correct_answer(die_1, die_2, questiontype)
